...
export class ExternalModuleService {
    ...
    ): Promise<ErrorInfo | AcquireTokenOutput> {
    ...
    async createjudge(req: CreateJudgeRequest): Promise<void> {
        ...
        const taskid: number =
            parseInt((await this.redisService.client.get("Taskids")) || "0") +
            1;
    ...
    //Exit，控制端要求下线
    async Exit(wsId: string, { reason }: ExitArgs): Promise<void> {
        await this.gateway.forceDisconnect(wsId, reason || "no reason");
        console.log("控制端要求下线");
    }

    async getLog(wsId: string): Promise<string> {
