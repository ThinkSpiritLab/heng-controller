    ...
    source.forEach(src=>{
        for(const key in src){
            ...
            if(typeof target[key]!=="object"){
                ...
                deepAssign(<Record<string, unknown>>target[key],<Record<string, unknown>>src[key]);
            }
        }
    });
}
...
function vaildObject(obj:Record<string,unknown>):void{
    ...
    const profileName = obj[PROFILE_NAME_MAGIC]?obj[PROFILE_NAME_MAGIC]:"配置文件";
...
export function Type(type:unknown):<T extends Profile>(target:T,key:string)=>void
{
