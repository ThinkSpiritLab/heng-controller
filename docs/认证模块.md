...
###### **KeyPairArrDTO**
   ...
   - 添加密钥对时的数据传输对象，考虑到使用单个KeyPair作为body，若需批量添加，则需要逐个发送请求。
...
crypto库中的`generateKeyPairSync()`,带有PUBLICKEY/SECETKEY等字样的前缀或后缀，需截取中间部分1到4行，取中间`KEY_LENGTH_NOT_ROOT` 位。
