...
# 外部系统引入
`src/judger/judger.gateway.ts`中搜索关键字“评测机池交互”，填充`removeJudger`、`addJudger`、`releaseJudger`。调用`src/judger/judger.service.ts`中的`distributeTask`为评测机分配一个任务分配任务，无需自行记录为评测机分配了哪些任务。

`src/judger/judger.gateway.ts`搜索关键字“设置任务队列键名”，然后设置任务队列键名。
